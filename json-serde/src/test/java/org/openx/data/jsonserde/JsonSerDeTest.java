...
public class JsonSerDeTest {
    ...
    public void testSerializeWithMapping() throws SerDeException, JSONException {
        ...
        // No matter the order it comes back in
        assertEquals(
                new HashSet<>(Arrays.asList(obj.toString().substring(1, obj.toString().length()).substring(0, obj.toString().length() - 2).split(","))),
                new HashSet<>(Arrays.asList(res.substring(1, res.length()).substring(0, res.length() - 2).split(",")))
        );
